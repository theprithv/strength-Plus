...
const Login = () => {
  ...
  const [isTakingLong, setIsTakingLong] = useState(false); // Neon cold start feedback
  const [passwordStrength, setPasswordStrength] = useState({
    score: 0,
    label: "",
  });

  // Helper to manage loading state with delay feedback
  const startLoading = () => {
    setLoading(true);
    setError("");
    // If request takes longer than 2s, show "Connecting..." message
    const timer = setTimeout(() => setIsTakingLong(true), 2000);
    return timer;
  };
      ...
      <div className="login-auth-section">
        <div className="login-auth-container">
              ...
              <form 
                ...
                noValidate
                style={{ opacity: loading ? 0.7 : 1, transition: "opacity 0.2s", pointerEvents: loading ? "none" : "auto" }}
              >
                {isSignUp && (
